"use strict";
(self["webpackChunkapp"] = self["webpackChunkapp"] || []).push([["node_modules_cmx-web-components_core_dist_esm_cwc-chart_entry_js"],{
...
/***/ 53477:
/*!***************************************************************************!*\
  !*** ./node_modules/@cmx-web-components/core/dist/esm/cwc-chart.entry.js ***!
  \***************************************************************************/
/***/ ((__unused_webpack_module, __webpack_exports__, __webpack_require__) => {

__webpack_require__.r(__webpack_exports__);
/* harmony export */ __webpack_require__.d(__webpack_exports__, {
/* harmony export */   "cwc_chart": () => (/* binding */ Chart)
/* harmony export */ });
...
function brush$1(dim) {
  ...
  function started(event) {
    ...
    if (type === "overlay") {
      ...
      state.selection = selection = [[w0 = dim === Y ? W : min$1(pts[0][0], pts[1][0]), n0 = dim === X ? N : min$1(pts[0][1], pts[1][1])], [e0 = dim === Y ? E : max$1(pts[0][0], pts[1][0]), s0 = dim === X ? S : max$1(pts[0][1], pts[1][1])]];
        ...
        case MODE_HANDLE:
          {
            if (points[1]) {
              ...
              if (signX < 0) dx = max$1(W - w0, min$1(E - w0, dx)), w1 = w0 + dx, e1 = e0;else if (signX > 0) dx = max$1(W - e0, min$1(E - e0, dx)), w1 = w0, e1 = e0 + dx;
              if (signY < 0) dy = max$1(N - n0, min$1(S - n0, dy)), n1 = n0 + dy, s1 = s0;else if (signY > 0) dy = max$1(N - s0, min$1(S - s0, dy)), n1 = n0, s1 = s0 + dy;
            }
...
function ribbon(headRadius) {
  ...
  function ribbon() {
    ...
    if (ap > epsilon$1) {
      if (abs$1(sa1 - sa0) > ap * 2 + epsilon$1) sa1 > sa0 ? (sa0 += ap, sa1 -= ap) : (sa0 -= ap, sa1 += ap);else sa0 = sa1 = (sa0 + sa1) / 2;
      if (abs$1(ta1 - ta0) > ap * 2 + epsilon$1) ta1 > ta0 ? (ta0 += ap, ta1 -= ap) : (ta0 -= ap, ta1 += ap);else ta0 = ta1 = (ta0 + ta1) / 2;
    }
...
const resulterrbound = (3 + 8 * epsilon$3) * epsilon$3; // fast_expansion_sum_zeroelim routine from oritinal code
...
class Path$1 {
  ...
  arc(x, y, r) {
    ...
    if (this._x1 === null) this._ += `M${x0},${y0}`;else if (Math.abs(this._x1 - x0) > epsilon$4 || Math.abs(this._y1 - y0) > epsilon$4) this._ += "L" + x0 + "," + y0;
    if (!r) return;
...
var boundsStream = {
  ...
  polygonEnd: function () {
    ...
    if (areaRingSum < 0) lambda0$1 = -(lambda1 = 180), phi0 = -(phi1 = 90);else if (deltaSum > epsilon$5) phi1 = 90;else if (deltaSum < -epsilon$5) phi0 = -90;
...
const clipAntimeridian = clip(function () {
  return true;
...
function clipAntimeridianIntersect(lambda0, phi0, lambda1, phi1) {
  ...
  return abs$2(sinLambda0Lambda1) > epsilon$5 ? atan((sin$1(phi0) * (cosPhi1 = cos$1(phi1)) * sin$1(lambda1) - sin$1(phi1) * (cosPhi0 = cos$1(phi0)) * sin$1(lambda0)) / (cosPhi0 * cosPhi1 * sinLambda0Lambda1)) : (phi0 + phi1) / 2;
}
...
function clipCircle(radius) {
  ...
  function intersect(a, b, two) {
    ...
    if (meridian ? polar ? phi0 + phi1 > 0 ^ q[1] < (abs$2(q[0] - lambda0) < epsilon$5 ? phi0 : phi1) : phi0 <= q[1] && q[1] <= phi1 : delta > pi$3 ^ (lambda0 <= q[0] && q[0] <= lambda1)) {
...
function clipRectangle(x0, y0, x1, y1) {
  ...
  function corner(p, direction) {
    return abs$2(p[0] - x0) < epsilon$5 ? direction > 0 ? 0 : 3 : abs$2(p[0] - x1) < epsilon$5 ? direction > 0 ? 2 : 1 : abs$2(p[1] - y0) < epsilon$5 ? direction > 0 ? 1 : 0 : direction > 0 ? 3 : 2; // abs(p[1] - y1) < epsilon
  }
...
function graticule() {
  ...
  return graticule.extentMajor([[-180, -90 + epsilon$5], [180, 90 - epsilon$5]]).extentMinor([[-180, -80 - epsilon$5], [180, 80 + epsilon$5]]);
}
...
function conicConformalRaw(y0, y1) {
  ...
  function project(x, y) {
    if (f > 0) {
      if (y < -halfPi$2 + epsilon$5) y = -halfPi$2 + epsilon$5;
    } else {
      if (y > halfPi$2 - epsilon$5) y = halfPi$2 - epsilon$5;
    }
...
function orthographic() {
  return projection(orthographicRaw).scale(249.5).clipAngle(90 + epsilon$5);
}
...
}(defaultSource$1); // https://en.wikipedia.org/wiki/Linear_congruential_generator#Parameters_in_common_use
...
function arc() {
  ...
  function arc() {
    ...
    if (!(r1 > epsilon$6)) context.moveTo(0, 0); // Or is it a circle or annulus?
      ...
      if (rp > epsilon$6) {
        ...
        if ((da0 -= p0 * 2) > epsilon$6) p0 *= cw ? 1 : -1, a00 += p0, a10 -= p0;else da0 = 0, a00 = a10 = (a0 + a1) / 2;
        if ((da1 -= p1 * 2) > epsilon$6) p1 *= cw ? 1 : -1, a01 += p1, a11 -= p1;else da1 = 0, a01 = a11 = (a0 + a1) / 2;
      }
      ...
      if (!(da1 > epsilon$6)) context.moveTo(x01, y01); // Does the sector’s outer ring have rounded corners?
      ...
      if (!(r0 > epsilon$6) || !(da0 > epsilon$6)) context.lineTo(x10, y10); // Does the sector’s inner ring (or point) have rounded corners?
...
const d3 = /*#__PURE__*/Object.freeze({
  __proto__: null,
  bisect: bisectRight,
...
const Chart = class {
  constructor(hostRef) {
    (0,_core_cf426fab_js__WEBPACK_IMPORTED_MODULE_1__.r)(this, hostRef); // Reference to the d3 instance
    ...
    this.chartColor = ['bright-green'];
    /**
     * Chart's height in pixels
     */
    ...
    this.horizontalGridLines = 10;
    /**
     * Frequency of vertical axis label frequency
     */

    this.verticalAxisFrequency = 1;
    /**
     * Frequency of vertical axis label frequency
     */

    this.horizontalAxisFrequency = 1;
  }
  /**
   * Method to get the d3 library
   */
  ...
  getD3() {
    ...
    return (0,D_Documentos_Documentos_DocumentosPersonales_Cemex_appcemex_node_modules_babel_runtime_helpers_esm_asyncToGenerator_js__WEBPACK_IMPORTED_MODULE_0__["default"])(function* () {
      return _this.d3Library;
    })();
  }
  /**
   * Method to get the wrapper element
   */
  ...
  resizeChart() {
    this.width = this.host.getBoundingClientRect().width;
  }
  ...
  createChart(variant) {
    if (this.data) {
      ...
      if (this.legend || this.variant === 'pie') {
        this.container.append(this.renderLegend());
      }
    }
  }
  ...
  verticalBarsChart() {
    ...
    const x = band().domain(this.data.map(d => d.name)).range([this.margin.left, this.width - this.margin.right]).padding(0.3);
    const y = linear$2().domain([0, max(this.data, d => d.value)]).nice().range([this.height - this.margin.bottom, this.margin.top]);
    const svg = create('svg').attr('preserveAspectRatio', 'xMaxYMax meet').attr('viewBox', [0, 0, this.width, this.height]).attr('class', 'chart');

    const xAxis = g => g.attr('transform', `translate(0,${this.height - this.margin.bottom})`).attr('class', 'xAxis').call(axisBottom(x).tickSizeOuter(0).tickFormat((interval, i) => {
      return i % this.horizontalAxisFrequency !== 0 ? ' ' : interval;
    }));

    const yAxis = g => g.attr('transform', `translate(${this.margin.left},0)`).call(axisLeft(y).tickFormat((interval, i) => {
      return i % this.verticalAxisFrequency !== 0 ? ' ' : interval;
    })).call(i => i.select('.domain').remove()).call(j => j.append('text').attr('x', -this.margin.left).attr('y', 16).attr('text-anchor', 'start').text(this.chartTitle).attr('class', 'title')).call(k => k.append('text').attr('x', -this.margin.left).attr('y', this.height / 2 + 12).attr('text-anchor', 'middle').text(`${this.prefixUnits ? this.prefixUnits : this.suffixUnits ? this.suffixUnits : ''}`).attr('class', 'units').attr('transform', `translate( ${-(this.height / 2 + this.margin.left)} , ${this.height / 2 - this.margin.top}),` + 'rotate(-90)')).attr('class', 'yAxis');

    const yGridLines = g => {
      g.attr('class', 'grid').attr('transform', `translate(${this.margin.left},0)`).call(axisLeft(y).ticks(this.horizontalGridLines) // number of ticks, not interval
      .tickSize(-this.width + this.margin.left + this.margin.right) // ticks normally extend 'out' of the chart. - makes these extend 'in'
      .tickFormat(''));
    }; // grid line first, so they display behind the bars


    svg.append('g').call(yGridLines);
    svg.append('g').selectAll('rect').data(this.data).join('rect').attr('x', d => x(d.name)).attr('y', d => y(d.value)).attr('height', d => y(0) - y(d.value)).attr('width', x.bandwidth()).attr('class', `fill--${this.chartColor[0]}`);
  ...
  lineChart() {
    // remove old chart
    this.container.innerHTML = '';
    const x = point().domain(this.data.map(d => d.name)).range([this.margin.left, this.width - this.margin.right]);
    const y = linear$2().domain([0, max(this.data, d => d.value)]).nice().range([this.height - this.margin.bottom, this.margin.top]);
    const svg = create('svg').attr('preserveAspectRatio', 'xMaxYMax meet').attr('viewBox', [0, 0, this.width, this.height]).attr('class', 'chart');
    ...
    const yAxis = g => g.attr('transform', `translate(${this.margin.left},0)`).call(axisLeft(y).tickFormat((interval, i) => {
      return i % this.verticalAxisFrequency !== 0 ? ' ' : interval;
    })).call(i => i.select('.domain').remove()).call(j => j.append('text').attr('x', -this.margin.left).attr('y', 16).attr('fill', 'currentColor').attr('text-anchor', 'start').text(this.chartTitle).attr('class', 'title')).call(k => k.append('text').attr('x', -this.margin.left).attr('y', this.height / 2 + 12).attr('text-anchor', 'middle').text(`${this.prefixUnits ? this.prefixUnits : this.suffixUnits ? this.suffixUnits : ''}`).attr('class', 'units').attr('transform', `translate( ${-(this.height / 2 + this.margin.left)} , ${this.height / 2 - this.margin.top}),` + 'rotate(-90)')).attr('class', 'yAxis');
    ...
    const line$1 = line().defined(d => d.value).x(d => x(d.name)).y(d => y(d.value)).curve(cardinal); // grid line first, so they display behind the bars

    svg.append('g').call(yGridLines);
    svg.append('g').call(xAxis);
    svg.append('g').call(yAxis); // line

    svg.append('path').attr('class', `area-line stroke--${this.chartColor[0]}`).attr('fill', 'transparent').attr('stroke-width', '3px').attr('d', line$1(this.data)); // linechart circles

    svg.selectAll('circle').data(this.data).enter().append('circle').attr('class', `fill--${this.chartColor[0]}`).attr('stroke', '#fff').attr('cx', d => x(d.name)).attr('cy', d => y(d.value)).attr('r', '4');
    return svg.node();
  }
  ...
  areaChart() {
    ...
    const line$1 = line().defined(d => d.value).x(d => x(d.name)).y(d => y(d.value)).curve(curveLinear);
    const curve = curveLinear;
    const area = area$3().curve(curve).x(d => x(d.name)).y0(y(0)).y1(d => y(d.value)); // grid line first, so they display behind the bars

    svg.append('g').call(yGridLines);
    svg.append('g').call(xAxis);
    svg.append('g').call(yAxis); // line

    svg.append('path').attr('class', `area-line stroke--${this.chartColor[0]}`).attr('fill', 'transparent').attr('stroke-width', '3px').attr('opacity', '.5').attr('d', line$1(this.data)); // linechart circles

    svg.selectAll('circle').data(this.data).enter().append('circle').attr('class', `fill--${this.chartColor[0]}`).attr('stroke', '#fff').attr('cx', d => x(d.name)).attr('cy', d => y(d.value)).attr('r', '4'); // area

    svg.append('path').datum(this.data).attr('class', `fill--${this.chartColor[0]}`).attr('opacity', '.25').attr('d', area);
    return svg.node();
  }
  ...
  pieChart() {
    // remove old chart
    this.container.innerHTML = '';
    const svg = create('svg').attr('preserveAspectRatio', 'xMaxYMax meet').attr('viewBox', [-this.width / 2, -this.height / 2, this.width, this.height]).attr('class', 'chart');
    const arc$1 = arc().innerRadius(0).outerRadius(Math.min(this.width, this.height) / 2.5 - 1);
    const radius = Math.min(this.width, this.height) / 2 * 0.9;
    const arcLabel = arc().innerRadius(radius).outerRadius(radius);
    const pie$1 = pie().sort(null).value(d => d.value);
    const arcs = pie$1(this.data); // transform array to have correct class name
    ...
    const color = ordinal(transformedArray);

    const yAxis = g => g.attr('transform', `translate(${-this.width / 2},${-this.height / 2})`).call(j => j.append('text').attr('x', 0).attr('y', 16).attr('fill', 'currentColor').attr('text-anchor', 'start').text(this.chartTitle).attr('class', 'title'));

    svg.append('g').attr('stroke', 'transparent').selectAll('path').data(arcs).join('path').attr('class', color).attr('d', arc$1).append('title').text(d => `${d.value}`);
    svg.append('g').attr('font-size', 16).attr('text-anchor', 'middle').selectAll('text').data(arcs).join('text').attr('transform', d => `translate(${arcLabel.centroid(d)})`).call(text => text.filter(d => d.endAngle - d.startAngle > 0.25).append('tspan').attr('x', 0).attr('y', '0.7em').text(d => `${this.prefixUnits ? this.prefixUnits : ''}` + d.value + `${this.suffixUnits ? this.suffixUnits : ''}`));
    svg.append('g').call(yAxis);
    return svg.node();
  }
  ...
  renderLegend() {
    ...
    wrapper.classList.add('cwc-chart__legend');

    if (this.variant === 'pie') {
      for (let index = 0; index < this.data.length; index++) {
        const wrapperItem = document.createElement('div');
        wrapperItem.classList.add('cwc-chart__legend-item');
        const rect = document.createElement('span');
        rect.classList.add('cwc-chart__legend-rect');
        rect.classList.add(`bg--${this.chartColor[index]}`);
        const text = document.createElement('span');
        text.classList.add('cwc-chart__legend-text');
        text.innerHTML = this.data[index].name;
        wrapperItem.append(rect);
        wrapperItem.append(text);
      ...
      const rect = document.createElement('span');
      rect.classList.add('cwc-chart__legend-rect');
      rect.classList.add(`bg--${this.chartColor[0]}`);
      const text = document.createElement('span');
      text.classList.add('cwc-chart__legend-text');
      text.innerHTML = this.legend;
      wrapperItem.append(rect);
      wrapperItem.append(text);
      wrapper.append(wrapperItem);
    }
  ...
  render() {
    return (0,_core_cf426fab_js__WEBPACK_IMPORTED_MODULE_1__.h)(_core_cf426fab_js__WEBPACK_IMPORTED_MODULE_1__.H, null, (0,_core_cf426fab_js__WEBPACK_IMPORTED_MODULE_1__.h)("div", {
      ...
      class: `${this.variant === 'custom' ? '' : 'cwc-chart'} ${this.designVersion} ${this.data ? '' : 'loading'}`
    }, this.variant === 'custom' ? (0,_core_cf426fab_js__WEBPACK_IMPORTED_MODULE_1__.h)("slot", null) : null, this.variant === 'custom' || this.data ? null : (0,_core_cf426fab_js__WEBPACK_IMPORTED_MODULE_1__.h)("cwc-loader", {
      delayMessageTime: 0
...
//# sourceMappingURL=node_modules_cmx-web-components_core_dist_esm_cwc-chart_entry_js.js.map